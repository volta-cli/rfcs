...
The core change to Volta will be expanding our `Project` model to support a _sequence_ of project roots, instead of a single project root. We will support a sequence by adding the ability to put an `"extends"` entry inside the `"volta"` section of `package.json`. The value of that entry will be the path to another json file, which we will parse just like the original `package.json`. The directory that new file is in will be the next value in the sequence of project roots. Then, if that new file also has an `"extends"` entry, we will continue the same process, building up effectively a linked list of project roots to use for all of our project information.
...
To support a use-case where a project wants to exclude e.g. `yarn` and _not_ allow a later project file to add one, we will support a new value for the package manager keys in the `"volta"` section: `false`. Setting `"yarn": false` will mean that no Yarn is available _and_ that a file later in the sequence with a `yarn` setting won't be able to override that removal of Yarn.
